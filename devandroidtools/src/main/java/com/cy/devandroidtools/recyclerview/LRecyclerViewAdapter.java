...
import com.cy.devandroidtools.recyclerview.interfaces.OnItemLongClickListener;
...
public class LRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TYPE_REFRESH_HEADER) {
            return new ViewHolder(mRefreshHeader.getHeaderView().getContext() ,mRefreshHeader.getHeaderView());
        } else if (isHeaderType(viewType)) {
            return new ViewHolder(getHeaderViewByType(viewType).getContext() ,getHeaderViewByType(viewType));
        } else if (viewType == TYPE_FOOTER_VIEW) {
            return new ViewHolder(mFooterViews.get(0).getContext() , mFooterViews.get(0));
        }
...
//                    	System.out.println("mSpanSizeLookup1 = " +gridManager.getSpanCount() );
//                        return (isHeader(position) || isFooter(position) || isRefreshHeader(position))
//                                ? gridManager.getSpanCount() : 1;
//                    } else {
//                    	System.out.println("mSpanSizeLookup2 = " +gridManager.getSpanCount() + " else = " + mSpanSizeLookup.getSpanSize(gridManager,  (position - (getHeaderViewsCount() + 1))));
//                        return (isHeader(position) || isFooter(position) || isRefreshHeader(position))
